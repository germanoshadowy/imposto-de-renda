function salarioL() {
    ...
    var ldt = document.createElement("tr")
    ...
    if(nome == "" || salarioBruto == "" || salarioBruto == NaN) return alerta();
    if(salarioBruto == NaN) return notAnumber();

    function alerta(){
        alert("Você não digitou um nome e/ou salário bruto, ou o valor digitado não é um número válido!");
    }
    
      ldt.appendChild(colunaNome)
      ldt.appendChild(colunaInss)
      ldt.appendChild(colunaIrpf)
      ldt.appendChild(colunaSalarioBruto)
      ldt.appendChild(colunaSalarioLiquido)
    ...
    var element = document.getElementById("tabelinha")
    element.appendChild(ldt)
  }
